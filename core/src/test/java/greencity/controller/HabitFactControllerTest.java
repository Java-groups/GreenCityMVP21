...
public class HabitFactControllerTest {
    ...
    @Test
    void deleteByUnexistedIdSuccessAndGetOk() throws Exception {
        when(habitFactService.delete(1L))
                .thenThrow(new NotDeletedException(ErrorMessage.HABIT_FACT_NOT_DELETED_BY_ID))
                .thenReturn(1L);
        mockMvc.perform(delete(habbitFactLink + "/{id}", 1L)).andExpect(status().isBadRequest());
        verify(habitFactService, times(1)).delete(1L);
    }
